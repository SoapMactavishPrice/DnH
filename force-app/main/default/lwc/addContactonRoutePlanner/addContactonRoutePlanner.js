...
import getContactRecord from '@salesforce/apex/AddAccountContactToCampController.getContactRecord';
import saveCampMembers from '@salesforce/apex/AddAccountContactToCampController.saveCampMembers';

const COLS = [
    { label: 'Name', fieldName: 'curl', type: 'url', typeAttributes: { label: { fieldName: 'Name' } } },
    { label: 'Account', fieldName: 'aurl', type: 'url', typeAttributes: { label: { fieldName: 'Account' } } },
    { label: 'Industry Group', fieldName: 'IndustryGroup', type: 'text' },
    { label: 'MailingCity', fieldName: 'MailingCity', type: 'text' },
    { label: 'MailingState', fieldName: 'MailingState', type: 'text' },
    { label: 'Mobile', fieldName: 'Mobile', type: 'text' }

];

export default class AddCampaignContacts extends NavigationMixin(LightningElement) {
    ...
    handleGetLeads() {
        ...
        new Promise((resolve, reject) => {
            ...
            getContactRecord({
                ...
                console.log(dataObj);
                this.AllProductData = dataObj.contactList;
                this.ShowTableData = dataObj.contactList;
                this.tempForFilter = dataObj.contactList;
    ...
    handlePicklistChangeChange(event) {
        ...
        let tempNewData = [];
        if (event.currentTarget.dataset.name == 'industrygrp') {
            this.industryGroupValue = event.detail.value;
        } else if (event.currentTarget.dataset.name == 'leadsrc') {
